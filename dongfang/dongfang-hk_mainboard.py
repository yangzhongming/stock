...
url = "http://29.push2.eastmoney.com/api/qt/clist/get?cb=jQuery112405113725083420002_1590036979391&pn={}&pz=20&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=m:128+t:3&fields=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,f15,f16,f17,f18,f19,f20,f21,f23,f24,f25,f26,f22,f33,f11,f62,f128,f136,f115,f152&_=1590036979394"
...
def get_hk_mainboard(page):
    print("************ 获取页面数据处理开始：get_hk_mainboard **********：第" + page + "页")
    ...
    print("************ 获取页面数据处理结束：get_hk_mainboard **********：第" + page + "页")
    return list_reponse;

def save_hk_mainboard(stocks_list):
    print("************ 保存据处理开始：save_hk_mainboard **********：")
    try:
        ...
        db.rollback()
        db.close()
    print("************ 保存据处理结束：save_hk_mainboard **********：")
...
def main_hk_mainboard():
    for i in range(100,109): # 页数：108
        try:
            print("************ 主函数处理开始：main_hk_mainboard **********：第"+str(i)+"页")
            #1.获取主板数据
            stock_list = get_hk_mainboard(str(i))
            #2.主板数据保存至DB
            save_hk_mainboard(stock_list)
            print("************ 主函数处理结束：main_hk_mainboard **********：第"+str(i)+"页")
            #3.休眠
            time.sleep(5)
        except Exception as e:
            print(e)
