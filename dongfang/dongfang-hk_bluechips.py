...
url = "http://78.push2.eastmoney.com/api/qt/clist/get?cb=jQuery112404085463240992835_1590050992647&pn={}&pz=20&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=b:DLMK0104&fields=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,f15,f16,f17,f18,f19,f20,f21,f23,f24,f25,f26,f22,f33,f11,f62,f128,f136,f115,f152&_=1590050992648"
...
db = pymysql.connect(host='localhost',
                     ...
                     charset='utf8mb4')
cursor = db.cursor()
def get_hk_bluechips(page):
    print("************ 获取页面数据处理开始：get_hk_bluechips **********：第" + page + "页")
    ...
    print("************ 获取页面数据处理结束：get_hk_bluechips **********：第" + page + "页")
    return list_reponse;
...
# 查询主板信息
def select_hk_bluechips(data_list):

    sql_select_hk_bluechips = "select dm from hk_bluechips where dm = {}"
    cursor.execute(sql_select_hk_bluechips.format(data_list[0]))
    return cursor.fetchone()

# 更新主板信息
def update_hk_bluechips(data_list):

    dm = data_list[0]
    data_tmp = data_list[1:]
    sql_update_hk_mainborad = "update hk_bluechips set mc=%s,zxj=%s,zde=%s,zdf=%s,jk=%s,zg=%s,zd=%s,zs=%s,cjl=%s,cje=%s where dm={}"
    cursor.execute(sql_update_hk_mainborad.format(dm),data_tmp)
    db.commit()

# 插入主板信息
def insert_hk_bluechips(data_list):

    sql_insert_hk_bluechips = "insert into `hk_bluechips`" \
                 "(`dm`, `mc`, `zxj`, `zde`, `zdf`, `jk`, `zg`, `zd`, `zs`, `cjl`,`cje`) " \
                 "values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,% s)"
    cursor.execute(sql_insert_hk_bluechips, data_list)
    db.commit()
...
# 主板主函数
def main_hk_bluechips():
    for i in range(1,4):   # 页数：3
        try:
            print("************ 主函数处理开始：main_hk_bluechips **********：第"+str(i)+"页")
            #1.获取主板数据
            stock_list = get_hk_bluechips(str(i))
            #2.主板数据保存至DB
            save_hk_bluechips(stock_list)
            print("************ 主函数处理结束：main_hk_bluechips **********：第"+str(i)+"页")
            #3.休眠
            time.sleep(5)
        except Exception as e:
            print(e)
