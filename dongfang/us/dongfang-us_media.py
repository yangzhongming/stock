...
url = "http://46.push2.eastmoney.com/api/qt/clist/get?cb=jQuery112405990091845338901_1590383455492&pn={}&pz=20&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=b:MK0220&fields=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,f15,f16,f17,f18,f20,f21,f23,f24,f25,f26,f22,f33,f11,f62,f128,f136,f115,f152&_=1590383455562"
...
db = pymysql.connect(host='localhost',
                     ...
                     charset='utf8mb4')
cursor = db.cursor()
def get_us_media(page):
    print("************ 获取页面数据处理开始：get_us_media **********：第" + page + "页")
    ...
    print("************ 获取页面数据处理结束：get_us_media **********：第" + page + "页")
    return list_reponse;

def save_us_media(stocks_list):
    print("************ 保存据处理开始：save_us_media **********：")
    try:
        for index in range(len(stocks_list)):
            ...
            #判断是否有相同的ID
            bb = select_us_media(data_list)
            if (bb):
                # 如果存在就进行更新
                update_us_media(data_list)
            else:
                #如果不存在，就进行插入数据
                insert_us_media(data_list)
        ...
        db.rollback()
        db.close()
    print("************ 保存据处理结束：save_us_media **********：")
# 查询主板信息
def select_us_media(data_list):
    sql_select_us_media = "select dm from us_media where dm = '{}' "
    cursor.execute(sql_select_us_media.format(data_list[0]))
    return cursor.fetchone()

# 更新主板信息
def update_us_media(data_list):

    dm = data_list[0]
    data_tmp = data_list[1:]
    sql_update_hk_mainborad = "update us_media set mc=%s,zxj=%s,zde=%s,zdf=%s,jk=%s,zg=%s,zd=%s,zs=%s,cjl=%s,cje=%s,zsz=%s,syl=%s where dm='{}'"
    cursor.execute(sql_update_hk_mainborad.format(dm), data_tmp)
    db.commit()

# 插入主板信息
def insert_us_media(data_list):

    sql_insert_us_media = "insert into us_media" \
                 "(dm, mc, zxj, zde, zdf, jk, zg, zd, zs, cjl,cje,zsz,syl) " \
                 "values( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    cursor.execute(sql_insert_us_media, data_list)
    db.commit()
...
def main_us_media():
    for i in range(1,2): # 页数：1
        try:
            print("************ 主函数处理开始：main_us_media **********：第"+str(i)+"页")
            #1.获取主板数据
            stock_list = get_us_media(str(i))
            #2.主板数据保存至DB
            save_us_media(stock_list)
            print("************ 主函数处理结束：main_us_media **********：第"+str(i)+"页")
            #3.休眠
            time.sleep(5)
        except Exception as e:
            print(e)
